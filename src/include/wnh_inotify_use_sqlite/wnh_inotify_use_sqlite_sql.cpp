...
bool wnh_inotify_use_sqlite::fail_task_list_create_table() //创建一张数据表-失败任务列表
{
    ...
    string sql;
    sql = sql + "CREATE TABLE IF NOT EXISTS " + TAIL_TASK_LIST_TABLE_NAME + " (client_ip TEXT NOT NULL, event_id INT NOT NULL, src_path TEXT NOT NULL, dst_path TEXT NOT NULL, update_date DATE NOT NULL, PRIMARY KEY (client_ip, src_path));";
    return sqlite_op.sql_query(sql);
    //sql = "";
    //sql = sql + "REPLACE INTO task_list_table (client_ip, event_id, src_path, dst_path, update_date) VALUES ('0', '0', '0', '0', datetime(CURRENT_TIMESTAMP,'localtime'));";
    //return sqlite_op.sql_query(sql);
...
bool wnh_inotify_use_sqlite::add_fail_task_info(const string & client_ip, const string & event_id, const string & src_path) //添加一行失败任务信息
{
    ...
    string sql = "";
    sql = sql + "REPLACE INTO " + TAIL_TASK_LIST_TABLE_NAME + " (client_ip, event_id, src_path, dst_path, update_date) SELECT client_ip, event_id, src_path, dst_path, update_date FROM " + TASK_LIST_TABLE_NAME + " WHERE (client_ip = '" + client_ip + "' AND event_id = '" + event_id + "' AND src_path = '" + sqlite_op.sqlite_escape(src_path) + "');";
    if(!sqlite_op.sql_query(sql))
    {
        return false;
    }
