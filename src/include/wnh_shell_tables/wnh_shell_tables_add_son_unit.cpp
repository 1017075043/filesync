...
bool wnh_shell_tables::add_son_unit(const int & num) //添加子行
{
    ...
    for(int i = 0; i < tables_unit[num].num; i ++)
    {
        ...
        if(values_display_width > tables_unit[num].value_width[i])
        {
            ...
            tables_unit_son[local_son_line_num].push_back(new_str_temp);
        ...
        {
            tables_unit_son[local_son_line_num].push_back("");
        }
        //WNHINFO(tables_unit[num].value[i] << ", 长度:" << tables_unit[line_num].value[i].size() << ", 中文个数:" << get_chinese_num(tables_unit[line_num].value[i]) << ", 非中文个数:" << tables_unit[line_num].value[i].size() - get_chinese_num(tables_unit[line_num].value[i]) * 3 << ", 在shell显示占用的宽度：" << tables_unit[line_num].value[i].size() - get_chinese_num(tables_unit[line_num].value[i]));
    }
