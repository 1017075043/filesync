...
bool wnh_sqlite::sql_query(string sql) //执行一条不会有结果返回的sql语句
{
    sqlite_escape(sql);
...
bool wnh_sqlite::sql_query(string sql, string &result)//执行一条有一个结果返回的sql语句
{
    sqlite_escape(sql);
...
bool wnh_sqlite::sql_query(string sql, string *&result, int &ncolumn) //执行一条有多行结果返回的sql语句
{
    sqlite_escape(sql);
    char **azResult; //二维数组存放结果
    //查询数据
...
bool wnh_sqlite::sql_query(string sql, string **&result, int &nrow ,int &ncolumn) //执行一条有多行结果返回的sql语句
{
    sqlite_escape(sql);
    char **azResult; //二维数组存放结果
    //查询数据
...
string wnh_sqlite::sqlite_escape(string sql)
{
    //sql = replace_all_distinct(sql, "/", "//");
    sql = replace_all_distinct(sql, "'", "''");
    //sql = replace_all_distinct(sql, "[", "/[");
    //sql = replace_all_distinct(sql, "]", "/]");
    //sql = replace_all_distinct(sql, "%", "/%");
    //sql = replace_all_distinct(sql, "&","/&");
    //sql = replace_all_distinct(sql, "_", "/_");
    //sql = replace_all_distinct(sql, "(", "/(");
    //sql = replace_all_distinct(sql, ")", "/)");
    return sql;
}
