...
{
    if((unsigned int)info.length() != write(socket_num, info.c_str(), size))
    {
...
bool wnh_tcp_client::send_file_size_info(unsigned long file_size) //发送文件大小信息
{
    ...
    if(send_info(info))
    {
        ...
        {
            WNHDEBUG("成功发送文件大小信息, file_size:" << file_size << ", 并得到的正常回复");
            return true;
        }
        else
        {
            WNHERROR("成功发送文件大小信息, file_size:" << file_size << ", 但是并没得到得到的正常回复");
            return false;
        }
    }
    WNHERROR("发送文件大小信息失败了, file_size:" << file_size << ", errno=" << errno << ", mesg=" << strerror(errno));
    return false;
}
...
bool wnh_tcp_client::send_file(const string file_name) //发送一个文件
{
    ...
    send_file_content(file_name, file_size);
