...
bool wnh_tcp_client::accept_download_file_content_info(const string info, const unsigned long & file_size, const string & temp_file_name) //接收下载文件内容信息
{
    ...
    while(1)
    {
        ...
        if(!accept_info(info_bin, info_size))
        {
            WNHERROR("socket_num:" << socket_num << ", 接收下载文件内容信息失败了" << ", errno=" << errno << ", mesg=" << strerror(errno));
            return true;
        }
        //  if(info_size == 0)
        //  {
...
bool wnh_tcp_client::download_file(const string file_name, const string temp_file_name) //下载文件
{
    ...
    while(1)
    {
        ...
        {
            WNHERROR("socket_num:" << socket_num << ", 会话异常结束, errno=" << errno << ", mesg=" << strerror(errno));
            return true;
        }
    }
    return true;
...
bool wnh_tcp_client::send_apply_download_file_info(const string file_name) //发送请求文件下载信息
{
    ...
    if(send_info(info))
    {
        if(!accept_info(info))
        {
            WNHERROR("socket_num:" << socket_num << ", 发送请求文件下载信息失败了" << ", errno=" << errno << ", mesg=" << strerror(errno));
            return false;
        }
