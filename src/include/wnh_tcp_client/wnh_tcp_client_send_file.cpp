...
bool wnh_tcp_client::send_start_send_file_info() //发送开始发送文件信息
{
    if(send_info(WNH_TCP_START_SEND_FILE_INFO))
    {
        ...
        if(!accept_info(info))
        {
            WNHERROR("socket_num:" << socket_num << ", 发送开始发送文件信息失败了" << ", errno=" << errno << ", mesg=" << strerror(errno));
            return false;
        }
...
bool wnh_tcp_client::send_end_send_file_info() //发送结束发送文件信息
{
    if(send_info(WNH_TCP_END_SEND_FILE_INFO))
    {
        ...
        if(!accept_info(info))
        {
            WNHERROR("socket_num:" << socket_num << ", 发送结束发送文件信息失败了" << ", errno=" << errno << ", mesg=" << strerror(errno));
            return false;
        }
...
bool wnh_tcp_client::send_start_send_file_content_info() //发送开始发送文件内容信息
{
    if(send_info(WNH_TCP_START_SEND_FILE_CONTENT_INFO))
    {
        ...
        if(!accept_info(info))
        {
            WNHERROR("socket_num:" << socket_num << ", 发送开始发送文件内容信息失败了" << ", errno=" << errno << ", mesg=" << strerror(errno));
            return false;
        }
...
bool wnh_tcp_client::send_end_send_file_content_info() //发送结束发送文件内容信息
{
    if(send_info(WNH_TCP_END_SEND_FILE_CONTENT_INFO))
    {
        ...
        if(!accept_info(info))
        {
            WNHERROR("socket_num:" << socket_num << ", 发送结束发送文件内容信息失败了" << ", errno=" << errno << ", mesg=" << strerror(errno));
            return false;
        }
...
bool wnh_tcp_client::accept_send_file_content_result() //接收发送文件内容结果
{
    ...
    if(!accept_info(info))
    {
        WNHERROR("socket_num:" << socket_num << ", 接收发送文件内容结果失败了" << ", errno=" << errno << ", mesg=" << strerror(errno));
        return false;
    }
...
bool wnh_tcp_client::send_file_size_info(unsigned long file_size) //发送文件大小信息
{
    string info;
    ...
    if(send_info(info))
    {
        if(!accept_info(info))
        {
            WNHERROR("socket_num:" << socket_num << ", 发送文件大小信息失败了" << ", errno=" << errno << ", mesg=" << strerror(errno));
            return false;
        }
...
bool wnh_tcp_client::send_file_name_info(const string name) //发送文件名称信息
{
    string info;
    ...
    if(send_info(info))
    {
        if(!accept_info(info))
        {
            WNHERROR("socket_num:" << socket_num << ", 发送文件名称信息失败了" << ", errno=" << errno << ", mesg=" << strerror(errno));
            return false;
        }
