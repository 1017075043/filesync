...
{
    if((unsigned int)info.length() != write(nfp, info.c_str(), size))
    {
...
bool wnh_tcp_server::accept_file_size_info(const int nfp, const string info, unsigned long & file_size) //接收文件大小信息
{
    if(info.substr(0, strlen(WNH_TCP_SEND_FILE_SIZE_INFO)) == WNH_TCP_SEND_FILE_SIZE_INFO)
    {
        ...
        {
            WNHDEBUG("接收到文件大小信息, 文件大小:" << file_size << ", 并正常回复")
            return true;
        }
        WNHERROR("接收到文件大小信息, 文件大小:" << file_size << ", 但是回复时失败了");
    }
    return false;
}
...
bool wnh_tcp_server::accept_file_content(const int nfp, const string info, const string file_name, const unsigned long & file_size) //接收一个文件内容
{
    if(!accept_start_send_file_content_info(nfp, info))
    {
        return false;
...
bool wnh_tcp_server::accept_file(const int nfp, const string info) //接收一个文件
{
    ...
    while(1)
    {
        ...
        if(accept_file_content(nfp, info, "/tmp/test/b.txt", file_size))
        {
            continue;
        }
        WNHDEBUG("接收到无法识别的信息:" << info);
        send_info(nfp, info);
    }
    return false;
}
