...
int wnh_tcp_server::accept_apply_downinfo_file_info(const int nfp, const string info) //接收请求文件下载信息
{
    string file_name;
...
bool wnh_tcp_server::send_reply_downinfo_file_content(const int nfp, const string file_name, const unsigned long file_size) //接收请求文件下载的文件内容信息
{
    ...
    while(!file_info.eof())
    {
        ...
        {
            WNHINFO("发送文件:" << file_name << ", 进度:" << ((float)(((float)file_size - (float)temp_file_size)/(float)file_size))*100 << "%, 总大小:" << file_size << ", 剩余:" << temp_file_size);
            temp_time = LOCALTIMENUM;
        }

        if(temp_file_size == (unsigned long)0)
        {
            WNHINFO("发送文件:" << file_name << ", 进度:" << ((float)(((float)file_size - (float)temp_file_size)/(float)file_size))*100 << "%, 总大小:" << file_size << ", 剩余:" << temp_file_size);
            break;
        }
...
bool wnh_tcp_server::accept_downinfo_file_content_result(const int nfp) //接收下载文件成功接收一行文件内容信息结果
{
    string info;
    if(!accept_info(nfp, info))
    {
        WNHERROR("nfp:" << nfp << ", 接收下载文件成功接收一行文件内容信息结果失败了" << ", errno=" << errno << ", mesg=" << strerror(errno));
        return false;
    }
...
bool wnh_tcp_server::send_reply_downinfo_file_content_stop(const int nfp) //发送回复下载文件结束发送文件内容信息
{
    string info;
    if(send_info(nfp, WNH_TCP_APPLY_DOWNLOAD_FILE_INFO_CONTENT_STOP))
    {
        if(!accept_info(nfp, info))
        {
            WNHERROR("nfp:" << nfp << ", 发送回复下载文件结束发送文件内容信息失败了" << ", errno=" << errno << ", mesg=" << strerror(errno));
            return false;
        }
...
bool wnh_tcp_server::send_reply_downinfo_file_content_start(const int nfp) //发送回复下载文件开始发送文件内容信息
{
    string info;
    if(send_info(nfp, WNH_TCP_APPLY_DOWNLOAD_FILE_INFO_CONTENT_START))
    {
        if(!accept_info(nfp, info))
        {
            WNHERROR("nfp:" << nfp << ", 发送回复下载文件开始发送文件内容信息失败了" << ", errno=" << errno << ", mesg=" << strerror(errno));
            return false;
        }
...
bool wnh_tcp_server::accept_apply_downinfo_file_name_info(const int nfp, const string info, string & file_name) //接收请求文件下载的文件名字信息
{
    if(info.substr(0, strlen(WNH_TCP_APPLY_DOWNLOAD_FILE_INFO)) == WNH_TCP_APPLY_DOWNLOAD_FILE_INFO)
    {
        ...
        if(send_info(nfp, WNH_TCP_APPLY_DOWNLOAD_FILE_INFO))
        {
            ...
            {
                WNHERROR("nfp:" << nfp << ", 接收请求文件下载的文件名字信息失败了" << ", errno=" << errno << ", mesg=" << strerror(errno));
                return false;
            }
...
bool wnh_tcp_server::reply_downinfo_file_size_info(const int nfp, const string file_name, unsigned long & file_size) //回复请求文件下载文件大小
{
    wnh_system_operation file_attr;
    if(!file_attr.get_file_size(file_name, file_size) || to_string(file_size).empty())
    {
...
bool wnh_tcp_server::send_reply_downinfo_file_complete_info(const int nfp) //发送回复下载文件完成信息
{
    string info;
    if(send_info(nfp, WNH_TCP_APPLY_DOWNLOAD_FILE_INFO_COMPLETE))
    {
        if(!accept_info(nfp, info))
        {
            WNHERROR("nfp:" << nfp << ", 发送回复下载文件完成信息失败了" << ", errno=" << errno << ", mesg=" << strerror(errno));
            return false;
        }
...
bool wnh_tcp_server::send_reply_downinfo_file_szie_info(const int nfp, const unsigned long file_size) //发送回复下载文件大小信息
{
    string info;
    ...
    if(send_info(nfp, info))
    {
        if(!accept_info(nfp, info))
        {
            WNHERROR("nfp:" << nfp << ", 发送回复下载文件大小信息失败了" << ", errno=" << errno << ", mesg=" << strerror(errno));
            return false;
        }
...
bool wnh_tcp_server::send_reply_downinfo_file_error_stop_info(const int nfp) //发送回复下载文件错误停止信息
{
    string info;
    if(send_info(nfp, WNH_TCP_APPLY_DOWNLOAD_FILE_INFO_ERROR_END))
    {
        if(!accept_info(nfp, info))
        {
            WNHERROR("nfp:" << nfp << ", 发送回复下载文件错误停止信息失败了" << ", errno=" << errno << ", mesg=" << strerror(errno));
            return false;
        }
